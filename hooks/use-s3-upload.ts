...
const useS3Upload = (options: UseS3UploadOptions) => {
			...
			return new Promise(async (resolve) => {
				try {
					...
					if (!presignRes.ok) {
						...
						return resolve({
							name: file.name,
							message: err.error ?? "Failed to get upload URL",
						});
					}
	...
	const onDrop = useCallback(
		(acceptedFiles: File[], fileRejections: FileRejection[]) => {
			...
			if (autoUpload && validFiles.length > 0) {
				...
				Promise.all(validFiles.map(uploadFileWithProgress)).then(
					(responses) => {
						...
						const latestKey =
							responseSuccesses[responseSuccesses.length - 1]?.key;
	...
	const onUpload = useCallback(async () => {
		...
		const responses = await Promise.all(
			filesToUpload.map(uploadFileWithProgress),
		);
		...
		const responseSuccesses = responses.filter((x) => !x.message);
		const newSuccessNames = Array.from(
